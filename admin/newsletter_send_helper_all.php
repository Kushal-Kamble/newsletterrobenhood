...
function send_newsletter_now_all($newsletter_id, $conn, $BASE_URL) {
    ...
    $readFullUrl = rtrim($BASE_URL, '/') . "/public/newsletter_view.php?id=" . $newsletter_id;
    $readFullButton = "<div style='text-align:center;margin:18px 0'>
        <a href='".htmlspecialchars($readFullUrl)."' target='_blank' style='background:#fd5402;color:#fff;padding:10px 16px;border-radius:6px;text-decoration:none;display:inline-block;'>Read full newsletter</a>
    </div>";

    $html_body .= $readFullButton;
